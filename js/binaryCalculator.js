...
function checkOperator() {
    if(element.innerHTML.endsWith('+') || element.innerHTML.endsWith('-') || element.innerHTML.endsWith('*') || element.innerHTML.endsWith('/')) {
        return true;
    }
}
...
function clickSum() {
    if(element.innerHTML.length!=0 && !checkOperator()) {
        element.innerHTML += '+';
    }
}
...
function clickEql() {
    if(!checkOperator()) {
      let re = /\d+/g
      let re2 = /[\+\-\*\/]+/g
      let numbers = element.innerHTML.match(re);
      let operations = element.innerHTML.match(re2);
      ...
      element.innerHTML = numbers[0].toString(2);
    } else {
